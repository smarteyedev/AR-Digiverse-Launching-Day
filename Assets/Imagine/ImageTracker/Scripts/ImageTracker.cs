     ...
     void ParseData(string data)
{
    ...
    else if (trackerOrigin == TrackerOrigin.FIRST_TARGET_ORIGIN)
    {
        if (trackedIds[0] == id)
        {
            // Tetapkan target pertama sebagai referensi
...
// **Set Posisi Referensi (Bisa Hardcode atau Dynamic)**
Vector3 referencePos = new Vector3(0, 2, 1);
Quaternion referenceRot = Quaternion.Euler(0, 180, 0);

// **Metode untuk Menyimpan Posisi Saat Ini sebagai Referensi**
public void SetReferencePosition()
{
    referencePos = pos;
    referenceRot = rot;
    Debug.Log("Posisi Referensi Disimpan: " + referencePos + ", " + referenceRot.eulerAngles);
}
